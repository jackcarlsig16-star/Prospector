  ...
  const runBatch=async(rowsOverride)=>{
    ...
    for(let i=0;i<list.length;i+=1){
      ...
      await Promise.all(batch.map(async(acc,bi)=>{
        ...
        try{
          const parsed=await clientAssay({name:acc.name,web:acc.web,vert:acc.vert,customIntel:getActiveIntel(),exampleAccts:getActiveExamples(),stage:acc.stage||"Prospecting",relationshipType:acc.relationshipType});

...
const AccountCardActionBar = forwardRef(function AccountCardActionBar({
  ...
  const QUICK_GHOSTS = React.useMemo(() => [
  ...
  const askQuick=async()=>{
    ...
    try{
      const r=await fetch("/proxy/anthropic/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:MODELS.FAST,max_tokens:400,messages:[{role:"user",content:`Account: ${acc.name} | Stage: ${acc.stage||"Prospecting"} | Tier: ${acc.tier||"?"} | Vertical: ${acc.vert||"?"}\nBusiness model: ${acc.bm||"unknown"} | product fit: ${acc.pf||"unknown"}\nProducts: ${(acc.prods||[]).join(", ")||"none"} | Use cases: ${(acc.ucs||[]).join(", ")||"none"}\nCall history:\n${callSummaries||"None"}\n\nQ: ${quickQuery}\n\nAnswer in 2-3 sentences max, specific to this account.`}]})});

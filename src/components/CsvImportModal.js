...
import { getListsForBusiness, getAccountsForBusiness, bulkCreateAccountsForBusiness, linkAccountToLists, recordAccountActivity, createList } from '../utils/db';
...
function ListCheckboxes({ lists, selected, onToggle, onCreated }) {
    ...
    <div>
      ...
      <div style={{ display:"flex", gap:6 }}>
        <input value={newName} onChange={e=>setNewName(e.target.value)} placeholder="+ Create new list"
          onKeyDown={e=>e.key==='Enter' && newName.trim() && !creating && (async()=>{
            setCreating(true);
